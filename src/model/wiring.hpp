...
[[using gnu: flatten]] auto run(const config::walker &config, asio::io_context &service, zf_env &stack, auto &command_input, auto &command_output,
  ...
  const auto commands = [&](auto &command_input) noexcept -> boost::leaf::awaitable<boost::leaf::result<void>> {
    ...
    for(;;)
    {
      ...
      case "subscribe"_h:
        ...
        {
          BOOST_LEAF_CO_TRY(co_await with_trigger(entrypoint, logger, [&](auto &&trigger_map) noexcept -> boost::leaf::awaitable<void> {
            auto trigger
...
#  endif // defined(LINUX) && !defined(USE_TCPDIRECT)
  ...
  const auto trigger_
    = [](auto &&continuation, const network_clock::time_point &feed_timestamp, const feed::update &update,
         typename automata_type::automaton *automaton) noexcept { return (automaton->trigger)(continuation, feed_timestamp, update, automaton); };
  ...
  auto send_ = ({
    ...
    const auto disposable_payload = send["disposable_payload"_hs].get_or(false);
    const auto cooldown = send["cooldown"_hs].get_or(std::chrono::steady_clock::duration(1s));
...
#if defined(USE_TCPDIRECT)
auto with_trigger_path(const config::walker &config, asio::io_context &service, zf_env &stack, auto &command_input, auto &command_output,
...
#  if defined(USE_TCPDIRECT)
  return dynamic_subscription_test |= handle_packet_loss_test |= send_datagram_test
         |= hof::partial(run)(service, stack, command_input, command_output, logger, continuation, properties);
#  else  // defined(USE_TCPDIRECT)
  return dynamic_subscription_test |= handle_packet_loss_test |= send_datagram_test
...
#  if defined(USE_TCPDIRECT)
    return run(config, service, stack, command_input, command_output, logger, continuation, std::false_type {},
