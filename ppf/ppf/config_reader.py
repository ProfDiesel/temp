...
def make_walker(config, value) -> WalkerResult:
...
class Walker:
    ...
    def fields(self) -> Set[str]:
        if isinstance(self.value, dict):
            for key, value in self.value.keys():
                yield key, make_walker(self.__config, value)
    ...
    def _(self, value_as_object: dict, field:str):
        return make_walker(self.__config, value_as_object[field])
    ...
    def walk(self: Walker) -> Generator(Tuple['Walker', str, Value], None, None):
        for field, value in self.fields():
            yield self, field, value
            if isinstance(value, str):
                yield from Walker(config, value).walk()
            if (isinstance(value, tuple) and (len(value) > 1) and isinstance(value[0], str)):
                yield from WalkerSequence(config, value).walk()


@dataclass
class WalkerSequence:
    __slots__ = ('__config', '__name')
