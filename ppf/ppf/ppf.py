from .config_reader import parse, write as write_config, walker
...
class Fairy:
    ...
    @staticmethod
    def _get_subscription(name, instrument, serial) -> Config
        def encode(message):
            return b64encode(jsons.dumps(message).encode())
        return {name: dict(instrument = instrument, message=encode({'instrument': instrument, 'content': f'hit #{serial} !'}), datagram=encode({'instrument': instrument, 'content': f'datagram hit #{serial} !'}))}


    async def setup(self, *, loop=None):
        self.process = await asyncio.create_subprocess_shell(str(self.__config.executable), stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=False)
...
send.fd <- {down_fd};''').format(down_fd=down_socket.fileno())

        if subscribe:
            message, datagram = self._get_payload(instrument, serial)
            self.process.stdin.write('''\
...
''').format()
    ...
    async def send_payload(self, instrument: int):
        self.process.stdin.write(write_config('entrypoint', instrument, serial).encode() + b'\n\n')
        await self.process.stdin.drain()


    async def send_subscribe(self, instrument: int):
        self.process.stdin.write(write_config('entrypoint', instrument, serial).encode() + b'\n\n')
        await self.process.stdin.drain()
